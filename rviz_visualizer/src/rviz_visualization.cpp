...
//      message_publisher_.publish(msg);
    ...
    case visualization_msgs::InteractiveMarkerFeedback::MOUSE_UP:
      ...
      msg.data = s.str();
      if(feedback->marker_name == "plane_marker_1_1")
      {
        if(++hax%6==0)
          createNewPlane();
      }
...
void RVizVisualization::makeROImarkers()
{
  visualization_msgs::InteractiveMarker int_marker;
  int_marker.header.frame_id = "/base_link";
  ...
  int_marker.name = "ROI marker 1";
  int_marker.description = "ROI marker";
  visualization_msgs::InteractiveMarkerControl control;
  ...
  control.markers.push_back( makeBox(int_marker, 1, 0.1, 0.1 ));
  control.markers.push_back( makeBox(int_marker, 0.1, 1, 0.1 ));
  control.markers.push_back( makeBox(int_marker, 0.1, 0.1, 1 ));
  control.always_visible = true;
  int_marker.controls.push_back(control);
  interactive_marker_server_objects_.insert(int_marker);
  int_marker.name = "ROI marker 2";
  interactive_marker_server_objects_.insert(int_marker);
  interactive_marker_server_objects_.applyChanges();
}
                                                                                    ...
                                                                                    const std::string& name)
{
  ...
  //TODO move math stuff into separate function (convert planeEqToQuaternion) and document it properly
  //dot product
...
// use 2 marker positions to make a new plane
void RVizVisualization::createNewPlane()
{
  // create marker object
  visualization_msgs::Marker marker;
  marker.type = visualization_msgs::Marker::POINTS;
  ...
  const double origin_x = 0.524164;
  const double origin_y = 0.0929133;
  const double origin_z = 0.054522;
  ...
  marker.color.r = (float)rand()/(float)RAND_MAX;
  marker.color.g = (float)rand()/(float)RAND_MAX;
  marker.color.b = (float)rand()/(float)RAND_MAX;
  marker.color.a = 1.0f;
  marker.lifetime = ros::Duration(0);
  marker.scale.x = POINT_SIZE;
  marker.scale.y = POINT_SIZE;
  ...
  interactive_marker_server_objects_.erase("Cloud_18");
  interactive_marker_server_objects_.erase("Cloud_45");
